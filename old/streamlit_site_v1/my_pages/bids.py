    ...
    for i, row in team_df.iterrows():
        ...
        with cols[2]:
            if player_name in st.session_state['captains']  or 'WILD' in player_name:
                ...
                my_bid = st.number_input("Insert a number", value=init_bid, key=f"{team_name}-{player_name}", label_visibility='collapsed', disabled=True, step=500)
                ...
                my_bid = st.number_input("Insert a number", value=init_bid, key=f"{team_name}-{player_name}", label_visibility='collapsed', step=500)
...
def bids_page():
    ...
    max_diff = 250000# 100000 # 10 #5000
